<?php
    ...
    if(isset($_GET['caseID']) && is_numeric($_GET['caseID'])){
        ...
        $query = "SELECT l.* , ls.judge_id AS judgeID FROM Trial l 
        INNER JOIN Lawsuit ls ON l.lawsuit_id = ls.id
        WHERE ls.judge_id = '$userID' AND l.id = '$caseID'";
        if ($result = mysqli_query($db,$query)){
            $count = mysqli_num_rows($result);
            if ($count == 1) {
                // Case belongs to user.
                $query = "SELECT * FROM Trial WHERE (`id` = '$caseID' AND `current_status` = '0')";
                if ($result = mysqli_query($db,$query)){
                    $count = mysqli_num_rows($result);
                    if ($count == 1) {
                        ...
                        if ($_SERVER['REQUEST_METHOD'] === 'POST' ){
                            ...
                            if(isset(
                                ...
                                // Update the lawsuit.
                                $query = "UPDATE Trial SET 
                                `current_status` = '1',
                                `end_date` = '$finalization_date',
                                `final_decision` = '$final_decision'
                                WHERE `id` = $caseID LIMIT 1";

                                if (mysqli_query($db, $query)) {
                                    $msg = "Success! Trial has ended.";
                                    echo '<script>alert("' . $msg . '");window.location.replace("trials.php");</script>';
                                } else {
                        ...
                        $msg = "Error! This trial is not available for final decision.";
                        die($msg);
                    }
                } else {
                    // Case is not open.
                    $msg = "Error! Trial could not be found.";
                    die($msg);
                }
            } else {
                // Case does not belong to user.
                $msg = "Error! This is not your trial. Go home.";
                die($msg);
            }
        }
    } else {
...
<head>
    <meta charset="utf-8">
    <title>NJIS - Give a Final Decision for Your Trial</title>
    <meta name="description"
        content="Give a Decision for Trial - National Judiciary Informatics System">
        ...
        <section>
            <div class="container" style="margin-top:40px;">
                <div class="section-header">
                    <h2>Give a verdict for Trial #<?php echo $caseID;?></h2>
                    <p>Please fill the following input for your <b>final decision</b> on the trial. This trial will be ended after the decision is made.</p>
                </div>
                <div class="card-display" style="margin-top:0px;">
                    <h4 style="margin-bottom:0;">Final Decision</h4><br>
                    <div class="form">
                        <form action="verdict_trials.php?caseID=<?php echo $caseID; ?>" method="POST" role="form" class="contactForm">
                            <div class="row" id="conciliator-display">
                                <div class="col-lg-6">
                                    ...
                                    <div class="">
                                        <input class="signUpButton" type="submit" value="End The Trial" style="width:100%;">
                                    </div>
                                </div>
                            </div>
                        </form>
